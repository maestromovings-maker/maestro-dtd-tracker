...
function getBulkStatus(city: any) {
  ...
  if (city.freq === "weekly" || city.freq === "by-request") return { label:"Knock Anytime", color:C.green };
...
function BulkCalendarView() {
  ...
  return <div>
    ...
    <div style={{ display:"flex", gap:8, marginBottom:20, flexWrap:"wrap" as any }}>
      {[["Knock Now",C.green],["Anytime",C.gold],["Pickup Week",C.red],["Verify First",C.gray]].map(([l,c])=>(
      ...
      <div style={{ display:"flex", gap:6, flexWrap:"wrap" as any }}>
        {DESOTO_PICKUPS.map((w,i)=>{
          ...
          return <span key={i} style={{ padding:"3px 8px", borderRadius:6, fontSize:10,
            background:isCurrent?C.green:C.navyM, color:isCurrent?C.navy:C.gray,
            fontWeight:isCurrent?"bold":"normal" }}>{w}</span>;
        })}
      </div>
    </div>
        ...
        return <div key={city.city} style={{ background:C.navyL, border:`1px solid ${C.navyM}`,
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8, flexWrap:"wrap" as any, gap:8 }}>
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              ...
              <span style={{ fontSize:10, color:C.gray }}>ZIP {city.zips.slice(0,3).join(", ")}{city.zips.length>3?"...":""}</span>
            </div>
    ...
    <div style={{ marginTop:20, background:C.navyL, borderRadius:10, padding:"14px 18px", border:`1px solid ${C.navyM}` }}>
      ...
      <div style={{ fontSize:11, color:C.grayL, lineHeight:1.6 }}>
        Knock the week BEFORE bulk pickup. Avoid the week OF pickup and the week AFTER.
        Weekly cities like Garland and Mesquite are great targets anytime.
      </div>
    </div>
  </div>;
}

function MapView({ zones, onCreateZone }: { zones:any[], onCreateZone:(name:string,zip:string,rep:string,addresses:string[])=>void }) {
  const mapContainer = useRef<HTMLDivElement>(null);
  const mapRef = useRef<any>(null);
  const drawRef = useRef<any>(null);
  const [mapReady, setMapReady] = useState(false);
  const [step, setStep] = useState<"zip"|"draw"|"review">("zip");
  ...
  useEffect(()=>{
    ...
    const initMap = () => {
      ...
      drawScript.onload = () => {
        ...
        mapRef.current = new win.mapboxgl.Map({
          container: mapContainer.current!,
          style: "mapbox://styles/mapbox/dark-v11",
          center: [-96.7970, 32.7767],
          zoom: 10,
        });
        drawRef.current = new win.MapboxDraw({
          displayControlsDefault: false,
          controls: { polygon: true, trash: true },
        });
        mapRef.current.addControl(drawRef.current);
        mapRef.current.addControl(new win.mapboxgl.NavigationControl(), "top-right");
        mapRef.current.on("load", () => setMapReady(true));
        mapRef.current.on("draw.create", (e:any) => {
          setBoundaryCoords(e.features[0].geometry.coordinates[0]);
          setStep("review");
        });
      };
  ...
  async function searchZip() {
    ...
    setSearching(true);
    setStep("draw");
    const res = await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${zipInput}.json?country=US&types=postcode&access_token=${MAPBOX_TOKEN}`);
  ...
  async function findAddresses() {
    ...
    const res = await fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/address.json?bbox=${bbox}&types=address&limit=50&access_token=${MAPBOX_TOKEN}`);
    const data = await res.json();
    const addrs = (data.features||[]).map((f:any)=>f.place_name.split(",")[0]);
    setFoundAddresses(addrs.slice(0,30));
    setSearching(false);
  }

  useEffect(()=>{ if(step==="review" && boundaryCoords) findAddresses(); },[step,boundaryCoords]);

  function handleCreate() {
    if (!zoneName.trim() || !foundAddresses.length) return;
    onCreateZone(zoneName, zipInput, assignedRep, foundAddresses);
    setStep("zip");
    setZipInput("");
    setZoneName("");
    setFoundAddresses([]);
  ...
  return <div>
    <div style={{ marginBottom:16 }}>
      <div style={{ fontSize:18, fontWeight:"bold", color:C.gold, marginBottom:4 }}>Territory Map</div>
      <div style={{ fontSize:12, color:C.gray }}>Enter a zip, draw a boundary, auto-populate addresses</div>
    </div>
    <div style={{ display:"flex", gap:8, marginBottom:16 }}>
      {[["1","Enter ZIP","zip"],["2","Draw Area","draw"],["3","Review","review"]].map(([num,label,s])=>(
        <div key={s} style={{ display:"flex", alignItems:"center", gap:4, padding:"4px 12px", borderRadius:20,
          background:step===s?C.gold:C.navyL, color:step===s?C.navy:C.gray, fontSize:11 }}>
          <span style={{ fontWeight:"bold" }}>{num}</span> {label}
        </div>
      ))}
    </div>
    {step==="zip" && <div style={{ background:C.navyL, borderRadius:10, padding:16, marginBottom:16, border:`1px solid ${C.navyM}` }}>
      <div style={{ fontSize:12, color:C.grayL, marginBottom:10 }}>Enter the ZIP code you want to knock:</div>
      <div style={{ display:"flex", gap:8 }}>
        <input value={zipInput} onChange={e=>setZipInput(e.target.value)}
          onKeyDown={(e:any)=>e.key==="Enter"&&searchZip()}
        ...
        <Btn col={C.gold} onClick={searchZip} disabled={searching}>{searching?"Loading...":"Go"}</Btn>
      </div>
    </div>}
    {step==="draw" && <div style={{ background:"#1a2010", border:`1px solid ${C.green}`, borderRadius:10, padding:14, marginBottom:16 }}>
      <div style={{ fontSize:12, fontWeight:"bold", color:C.green, marginBottom:4 }}>Draw your territory</div>
      <div style={{ fontSize:11, color:C.grayL }}>Click the polygon tool on the map, draw your boundary, then double-click to close the shape.</div>
    </div>}
    <div ref={mapContainer} style={{ width:"100%", height:400, borderRadius:10, overflow:"hidden", border:`1px solid ${C.navyM}`, marginBottom:16 }} />
    {!mapReady && <div style={{ textAlign:"center" as any, color:C.gray, fontSize:12, marginBottom:16 }}>Loading map...</div>}
    {step==="review" && <div style={{ background:C.navyL, borderRadius:10, padding:16, border:`1px solid ${C.gold}` }}>
      <div style={{ fontSize:13, fontWeight:"bold", color:C.gold, marginBottom:12 }}>
        {searching ? "Finding addresses..." : `${foundAddresses.length} addresses found`}
      </div>
      {!searching && <>
        <Field label="Zone Name"><Input value={zoneName} onChange={(e:any)=>setZoneName(e.target.value)} placeholder="e.g. Oak Cliff South" /></Field>
        <Field label="Assign Rep">
          <div style={{ display:"flex", gap:6 }}>
            {REPS.map(rep=>(
              <button key={rep.id} onClick={()=>setAssignedRep(rep.id)}
                style={{ display:"flex", alignItems:"center", gap:5, flex:1, padding:"7px 8px",
                  borderRadius:8, border:`1px solid ${assignedRep===rep.id?rep.color:C.navyM}`,
                  background:assignedRep===rep.id?`${rep.color}22`:"transparent",
                  color:assignedRep===rep.id?rep.color:C.gray,
        ...
        <div style={{ maxHeight:200, overflowY:"auto" as any, marginBottom:12, background:C.navy, borderRadius:6, padding:"8px 12px" }}>
          {foundAddresses.map((addr,i)=>(
            <div key={i} style={{ fontSize:11, color:C.grayL, padding:"3px 0",
              borderBottom:i<foundAddresses.length-1?`1px solid ${C.navyM}`:"none" }}>{addr}</div>
          ))}
        </div>
        <div style={{ display:"flex", gap:8 }}>
          <Btn col={C.gold} onClick={handleCreate} full>Create Zone</Btn>
          <Btn col={C.gray} onClick={()=>{ setStep("draw"); drawRef.current?.deleteAll(); setBoundaryCoords(null); }} full>Redraw</Btn>
        </div>
      </>}
    </div>}
  </div>;
...
export default function App() {
  ...
  async function addZoneFromMap(name:string, zip:string, rep:string, addresses:string[]) {
    setSaving(true);
    const { data:z, error } = await supabase.from("zones")
      .insert({ name, zip, assigned_rep:rep })
      .select().single();
    if(!error&&z){
      await supabase.from("doors").insert(
        addresses.map(addr=>({ zone_id:z.id, address:addr, status:"untouched", contact:"", notes:"" }))
      );
      setActiveZoneId(z.id);
      setActiveView(VIEWS.zones);
  ...
  return <div style={{ minHeight:"100vh", background:C.navy, fontFamily:"'Georgia', serif", color:C.white, display:"flex", flexDirection:"column" }}>
    ...
    <div style={{ background:C.navyL, borderBottom:`2px solid ${C.gold}`, padding:"0 16px", flexShrink:0 }}>
      <div style={{ maxWidth:960, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:58 }}>
        ...
        <div style={{ display:"flex", gap:4, flexWrap:"wrap" as any }}>
          {[[VIEWS.zones,"Zones"],[VIEWS.route,`Route (${todayRoute.length})`],[VIEWS.reps,"Reps"],[VIEWS.bulk,"Bulk"],[VIEWS.map,"Map"]].map(([v,label])=>(
    ...
    <div style={{ flex:1, maxWidth:960, margin:"0 auto", width:"100%", padding:"20px 16px", boxSizing:"border-box" as any }}>
        ...
        <div style={{ display:"flex", gap:8, marginBottom:18, flexWrap:"wrap" as any, alignItems:"center" }}>
          {zones.map(z=>{
            ...
            return <button key={z.id} onClick={()=>setActiveZoneId(z.id)}
              ...
              {bs && <span style={{ fontSize:9, color:bs.color, background:`${bs.color}22`, padding:"1px 6px", borderRadius:8 }}>{bs.label.split(" ")[0]}</span>}
            </button>;
          })}
      ...
      {activeView===VIEWS.map&&<MapView zones={zones} onCreateZone={addZoneFromMap} />}
